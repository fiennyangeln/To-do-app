...
module.exports.listFilter = function(req,res,next){
  var query;
  if (req.params.status=="true"){
    query = Task.find({'status': true}).select('name status').sort('name');
  }
  else if (req.params.status=="false"){
    query = Task.find({'status': false}).select('name status').sort('name');
  }
  else {
    query = Task.find().select('name status').sort('name');
  }
  query.exec(function(err,tasks){
    if (err){
      return res.status(400).send({message: getErrorMessage(err)});
